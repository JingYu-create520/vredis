...
- 128 维均匀随机是图索引的**最坏情况**（真实 embedding 有簇结构时召回更高）；
- 邻居超限裁剪为简单距离截断，未实现论文 Algorithm 4 启发式选择——大规模下图质量
  受此影响（50k × 128 维 ef=100 实测召回 0.54；启发式选择已列入 Roadmap）；
- ef_search 需按数据规模调节（50k × 128 维：ef=100 → 召回 0.54 / QPS 822，
  ef=800 → 召回 0.93 / QPS 163）；小规模时 HNSW 常数开销主导、可能慢于暴力（1k 实测 0.6×）。
