...
| 项 | 结论 | 证据 |
|---|---|---|
| HNSW 召回率 ≥ 95% | ✅ 实测 **100%** | h11（1k 点）与 h12（10k 点，#[ignore]）：recall@10 = 1.0000（m=16, ef_construction=200, ef=100，暴力对照） |
| metric 一致性切换 | ✅ 满足 | t40（匹配 → HNSW 路径正确）、t41（不匹配 → 暴力 fallback）、h26（l2/cos 排序语义区分验证） |
| 重启后重建可用 | ✅ 满足 | t42 + h20（按 id 数值序 + 确定性 seed = fnv1a(key) 重建，结果与重建前一致） |
